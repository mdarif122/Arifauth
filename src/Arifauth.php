...
use Arifauth\Exception\InvalidArgumentException;
use Arifauth\Exception\UnexpectedValueException;
use Arifauth\Storage\StorageInterface;
use Arifauth\Logger\LoggerInterface;
use Arifauth\Logger\Logger;
use Arifauth\HttpClient\HttpClientInterface;

/**
 * Arifauth\Arifauth
 *
 * For ease of use of multiple providers, Arifauth implements the class Arifauth\Arifauth,
...
class Arifauth
{
    ...
    public function __construct(
        ...
        if (is_string($config) && file_exists($config)) {
            ...
            throw new InvalidArgumentException('Arifauth config does not exist on the given path.');
        }
     ...
     *
     * @return \Arifauth\Adapter\AdapterInterface
     * @throws InvalidArgumentException
    ...
    public function getAdapter($name)
    {
        ...
        $adapter = isset($config['adapter']) ? $config['adapter'] : sprintf('Arifauth\\Provider\\%s', $name);
            ...
            foreach ($fs as $file) {
                if (!$file->isDir()) {
                    ...
                    if ($name === mb_strtolower($provider)) {
                        $adapter = sprintf('Arifauth\\Provider\\%s', $provider);
                        break;
                    }
                }
            }
