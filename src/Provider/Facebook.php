 ...
 *
 *   $config = [
 *       'callback' => Arifauth\HttpClient\Util::getCurrentUrl(),
 *       'keys' => ['id' => '', 'secret' => ''],
 *       'scope' => 'email, user_status, user_posts',
 *       'exchange_by_expiry_days' => 45, // null for no token exchange
 *   ];
 *
 *   $adapter = new Arifauth\Provider\Facebook($config);
 *
 *   try {
 ...
 *       $response = $adapter->setUserStatus("Arifauth test message..");
 *   } catch (\Exception $e) {
 *       echo $e->getMessage() ;
 *   }
 */
class Facebook extends OAuth2
{
     ...
     * @throws \Arifauth\Exception\InvalidAccessTokenException
     */
